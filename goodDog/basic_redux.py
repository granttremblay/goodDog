...
def lvl2_data(obsid_list):
    ...
    for obs in obsid_list:
        if len(glob.glob(obs+'/*/*evt2.fits')) == 1:
            print('Found obsID repro directory for obs %s' % obs)
        else:
...
def WCS(obsid_list):
    ...
    if os.path.exists('reprojected_data/%s_reproj_evt.fits' % obsid_list[0]) is False:
        print('Reprojecting observations...')
        reproject_obs(infiles='*/repro/*evt2.fits', outroot='reprojected_data/', verbose=0)
